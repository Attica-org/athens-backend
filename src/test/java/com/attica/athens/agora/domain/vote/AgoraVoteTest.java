...
class AgoraVoteTest {
    ...
    private static final int EXCEED_VOTE_TIME = 30;
    ...
    void 실패_투표형식_유효하지않는투표타입() {
        ...
        //when & then
        thenThrownBy(() ->
                agora.checkAgoraVoteRequest(request))
    ...
    void 실패_투표시간초과_만료된시간전달() {
        //given
        Agora agora = Agora.builder().build();
        agora.endAgora();
        LocalDateTime expiredTime = agora.getEndTime().minusSeconds(EXCEED_VOTE_TIME);
        Duration duration = Duration.between(expiredTime, LocalDateTime.now());

        //when & then
        thenThrownBy(checkVoteTime(duration))
                .isInstanceOf(VoteTimeOutException.class)
                .hasMessage("Voting period has expired.");
    }

    public ThrowingCallable checkVoteTime(Duration duration) {
        return () -> {
